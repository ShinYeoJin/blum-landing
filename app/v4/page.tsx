...
/* ── Confirmed Blum official image URLs (crawled from blum.com/kr/ko/ & /products/product-world/) ── */
const IMG = {
  /* hero */
  hero1:      "https://www.blum.com/images/560/258/4215892/corporate/media/bilder/produkte/boxsysteme/lbx0458_ab_fot_fo_bau_-sall_-apr6i_-v1_4:3.jpg",
  /* brand story slides */
  brand1:     "https://www.blum.com/images/560/420/4215882/corporate/media/bilder/unternehmen/190523_ARNO_Blum_Interzum_0194_4:3.jpg",
  brand2:     "https://www.blum.com/images/560/258/4188512/corporate/media/bilder/unternehmen/IMG1146_AA_FOT_FO_BAU_-SALL_-AMC_-V1_4:3.jpg",
  brand3:     "https://www.blum.com/images/560/258/4214413/corporate/media/bilder/services/services-overview/keyvisual-services_4:3.jpg",
  /* stats bg */
  ...
  /* LEGRABOX */
  legrabox1:  "https://www.blum.com/images/560/258/4215299/corporate/media/bilder/produkte/boxsysteme/legrabox-design/me10782852_aa_fot_fo_bau_-sall_-amc_-v1_4:3.jpg",
...
/* ── Data ── */
/* Brand story slides (3 slides for the dot indicators) */
const CHAPTERS = [
  {
...
/* Featured products — fullscreen pinned scroll (Problem 5) */
const FEAT_PRODUCTS = [
  {
    name:  "AVENTOS",
    cat:   "리프트 시스템",
...
/* Key stats for the bridge section (Problem 2) */
const KEY_STATS = [
  { n: 1952,  s: "",    l: "창립 연도",      sub: "오스트리아 포어알베르크" },
  { n: 2441,  s: "M€",  l: "전 세계 매출",   sub: "2024/25" },
  { n: 9850,  s: "명",   l: "임직원",         sub: "전 세계" },
...
function GCounter({ to, suffix = "" }: { to: number; suffix?: string }) {
  ...
  useEffect(() => {
    if (!on) return;
    const s = performance.now(), dur = 2000;
    ...
    <div ref={ref} className={`v4-wipe-section${active ? " v4-wipe-go" : ""}`}
      ...
      <div className="v4-wipe-content" style={{ position: "relative", zIndex: 1 }}>
        ...
        <blockquote className="v4-font-serif"
          style={{ fontSize: "clamp(1.6rem,4vw,3.2rem)", fontWeight: 300, color: CREAM, lineHeight: 1.5, maxWidth: "760px", margin: "0 auto 36px", fontStyle: "italic" }}>
...
export default function V4() {
  ...
  /* Featured product fullscreen slides */
  const prodWrapRef              = useRef<HTMLDivElement>(null);
  ...
  /* Hero animation triggers after load screen exits */
  useEffect(() => {
    if (!loadOut) return;
  ...
  useEffect(() => {
    const onScroll = () => {
      const el = chapterWrapRef.current; if (!el) return;
  ...
  /* Product fullscreen scroll tracking */
  useEffect(() => {
    const onScroll = () => {
      const el = prodWrapRef.current; if (!el) return;
      const rect      = el.getBoundingClientRect();
      const scrollable = el.offsetHeight - window.innerHeight;
      if (scrollable <= 0) return;
      const progress = Math.max(0, Math.min(1, -rect.top / scrollable));
      setProdIdx(Math.min(FEAT_PRODUCTS.length - 1, Math.floor(progress * FEAT_PRODUCTS.length)));
    };
    window.addEventListener("scroll", onScroll, { passive: true });
    return () => window.removeEventListener("scroll", onScroll);
  }, []);
  ...
  /* Click dot → scroll to that chapter */
  const scrollToChapter = (idx: number) => {
    const el = chapterWrapRef.current; if (!el) return;
    const scrollable = el.offsetHeight - window.innerHeight;
    const target = el.offsetTop + (idx / CHAPTERS.length) * scrollable + 1;
  ...
  const css = `
    ...
    @keyframes v4-word-in    { from{opacity:0;transform:translateY(20px)} to{opacity:1;transform:none} }
    ...
    /* Chapter word-by-word title animation */
    .v4-ch-word { display:inline-block; }
    .v4-ch-word-active { animation:v4-word-in 0.6s cubic-bezier(0.16,1,0.3,1) both; }

    /* Product fullscreen slides */
    .v4-prod-img-zoom { transition:transform 1.2s cubic-bezier(0.25,1,0.5,1); }
    .v4-prod-slide-active .v4-prod-img-zoom { transform:scale(1.04); }

    /* Stats card hover */
    .v4-stat-card { transition:background 0.3s ease; }
    .v4-stat-card:hover { background:rgba(212,175,55,0.06) !important; }

    @media (max-width:768px) {
      .v4-prod-layout { flex-direction:column !important; }
      .v4-prod-img-side { width:100% !important; height:50vh !important; }
    }
  `;
  ...
  return (
    <div className="v4-font-sans" style={{ backgroundColor: NAVY, color: CREAM, overflowX: "hidden" }}>
      ...
      {/* ══ HERO ════════════════════════════════════════════════════════ */}
      {/* Problem 3: "moving" slides from right, "ideas." slides from left, subtitle/buttons stagger in */}
      <section style={{ position: "relative", height: "100vh", minHeight: "700px", display: "flex", alignItems: "flex-end", overflow: "hidden", backgroundColor: NAVY }}>
        <div style={{ position: "absolute", inset: 0 }}>
          <img src={IMG.hero1} alt="blum"
            style={{ width: "100%", height: "110%", objectFit: "cover", opacity: 0.38, transform: `translateY(${heroParallax}px)` }}
          ...
          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to right, rgba(13,17,23,0.8) 0%, transparent 65%)" }} />
        </div>
        ...
        <div style={{ position: "relative", zIndex: 10, maxWidth: "1280px", margin: "0 auto", padding: "0 2rem 7rem", width: "100%", opacity: heroOpacity }}>
          ...
          <div style={{
            ...
            transition: "opacity 0.8s ease 0ms",
          }}>
          ...
          {/* "moving" — slides in from right */}
          <h1 className="v4-font-serif" style={{ marginBottom: "1.5rem", lineHeight: 0.88, fontWeight: 300, fontSize: "clamp(4.5rem,13vw,11rem)", letterSpacing: "-0.01em", overflow: "hidden" }}>
            <span style={{
              color: CREAM, display: "block", overflow: "hidden",
              opacity: heroAnim ? 1 : 0,
              transform: heroAnim ? "none" : "translateX(80px)",
              transition: "opacity 0.9s cubic-bezier(0.16,1,0.3,1) 150ms, transform 0.9s cubic-bezier(0.16,1,0.3,1) 150ms",
            }}>moving</span>
            {/* "ideas." — slides in from left */}
            <span style={{
          ...
          <p style={{
            ...
            opacity: heroAnim ? 1 : 0,
            transform: heroAnim ? "none" : "translateY(20px)",
            transition: "opacity 0.8s ease 500ms, transform 0.8s ease 500ms",
          }}>
            편리함을 높이고 삶의 질을 향상시키는<br />고품질 가구용 피팅
          </p>

          {/* Buttons — sequential fade */}
          <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
            <Link href="#products" className="v4-btn" style={{
              color: NAVY, backgroundColor: GOLD, textDecoration: "none", fontSize: "11px",
              letterSpacing: "0.2em", textTransform: "uppercase", padding: "14px 30px", gap: "8px",
              opacity: heroAnim ? 1 : 0,
              transform: heroAnim ? "none" : "translateY(16px)",
              transition: "opacity 0.7s ease 680ms, transform 0.7s ease 680ms",
            }}>제품 살펴보기 →</Link>
            <Link href="#timeline" className="v4-btn" style={{
              color: CREAM, border: `1px solid ${LINE}`, textDecoration: "none", fontSize: "11px",
              letterSpacing: "0.2em", textTransform: "uppercase", padding: "14px 30px",
              opacity: heroAnim ? 1 : 0,
              transform: heroAnim ? "none" : "translateY(16px)",
      ...
      {/* ══ BRAND STORY — pinned slideshow (Problems 1 & 4) ═══════════ */}
      {/* Problem 1: dots are now clickable slide indicators */}
      {/* Problem 4: word-by-word title, parallax BG, text slide-up */}
      <div ref={chapterWrapRef} style={{ height: "300vh", backgroundColor: NAVY }}>
        <div style={{ position: "sticky", top: 0, height: "100vh", overflow: "hidden", backgroundColor: NAVY }}>
          ...
          {/* BG images with parallax scale */}
          {CHAPTERS.map((ch, i) => (
            <div key={i} style={{ position: "absolute", inset: 0, opacity: chapterIdx === i ? 1 : 0, transition: "opacity 1s ease" }}>
              <img src={ch.img} alt={ch.label}
                style={{
                  width: "100%", height: "115%", objectFit: "cover", opacity: 0.32,
                  transform: chapterIdx === i ? "scale(1.04) translateY(-2%)" : "scale(1)",
                  transition: "transform 6s cubic-bezier(0.25,1,0.5,1)",
                }}
                onError={(e) => { (e.target as HTMLImageElement).style.opacity = "0"; }} />
              <div style={{ position: "absolute", inset: 0, background: `linear-gradient(to right, ${NAVY} 36%, transparent 100%)` }} />
          ...
          {CHAPTERS.map((ch, i) => (
            <div key={i} style={{
              ...
              transform: chapterIdx === i ? "translateY(-50%)" : chapterIdx > i ? "translateY(calc(-50% - 80px))" : "translateY(calc(-50% + 80px))",
              opacity: chapterIdx === i ? 1 : 0,
              transition: "all 0.9s cubic-bezier(0.16,1,0.3,1)",
              maxWidth: "600px", zIndex: 10,
            }}>
              {/* Label — fades in (Problem 4) */}
              <p style={{
                fontSize: "9px", letterSpacing: "0.5em", textTransform: "uppercase",
                color: `${GOLD}88`, marginBottom: "22px",
                opacity: chapterIdx === i ? 1 : 0,
                transition: "opacity 0.6s ease 0.1s",
              }}>{ch.label}</p>

              {/* Title — word by word (Problem 4) */}
              <h2 className="v4-font-serif" style={{ fontSize: "clamp(3rem,7vw,6rem)", fontWeight: 300, color: CREAM, lineHeight: 0.92, marginBottom: "28px", letterSpacing: "-0.01em" }}>
                {ch.title.split(" ").map((word, wi) => (
                  <span key={wi} className={`v4-ch-word${chapterIdx === i ? " v4-ch-word-active" : ""}`}
                    style={{ animationDelay: chapterIdx === i ? `${wi * 120 + 100}ms` : "0ms", marginRight: "0.25em" }}>
                    {word}
                  </span>
                ))}
              </h2>

              {/* Body — slides up (Problem 4) */}
              <p style={{
                fontSize: "15px", color: GRAY, lineHeight: 1.95, fontWeight: 300,
                opacity: chapterIdx === i ? 1 : 0,
                transform: chapterIdx === i ? "none" : "translateY(18px)",
                transition: "opacity 0.8s ease 0.4s, transform 0.8s ease 0.4s",
              }}>{ch.body}</p>
            </div>
          ))}

          {/* Problem 1: dot indicators — clickable, navigate to each slide */}
            ...
            {CHAPTERS.map((ch, i) => (
              <button
                ...
                style={{
                  ...
                  background:    chapterIdx === i ? GOLD : "transparent",
                  border:        `1.5px solid ${chapterIdx === i ? GOLD : `${GOLD}55`}`,
                  cursor:        "pointer", padding: 0,
                  transition:    "all 0.4s ease",
                }}
                aria-label={`슬라이드 ${i + 1}: ${ch.label}`}
              />
            ))}
          </div>
      ...
      {/* ══ KEY STATS BRIDGE — fills the black gap (Problem 2) ══════════ */}
      <section style={{ position: "relative", padding: "0", backgroundColor: "#060A0F", overflow: "hidden" }}>
        {/* Background image with overlay */}
        <div style={{ position: "absolute", inset: 0 }}>
          <img src={IMG.statsBg} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", opacity: 0.12 }}
            onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to bottom, #060A0F 0%, transparent 30%, transparent 70%, #060A0F 100%)" }} />
        </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "1px", backgroundColor: LINE }}>
            {KEY_STATS.map((s, i) => (
              <FadeIn key={s.l} delay={i * 100}>
                <div className="v4-stat-card" style={{ padding: "52px 32px", backgroundColor: "#060A0F" }}>
                  <div className="v4-font-serif" style={{ fontSize: "clamp(2.8rem,5vw,4.5rem)", fontWeight: 300, color: GOLD, lineHeight: 1, marginBottom: "10px" }}>
                    <GCounter to={s.n} suffix={s.s} />
                  </div>
                  <div style={{ fontSize: "14px", color: CREAM, marginBottom: "6px", fontWeight: 400 }}>{s.l}</div>
                  <div style={{ fontSize: "11px", color: GRAY, letterSpacing: "0.1em" }}>{s.sub}</div>
                </div>
              </FadeIn>
            ))}
          </div>
      ...
      {/* ══ PRODUCT WORLD — fullscreen pinned scroll (Problem 5) ════════ */}
      {/* dfy.co.kr FEATURED PROJECTS style: one product per screen, image right */}
      <section id="products" style={{ backgroundColor: "#0A0E14" }}>
        {/* Section header */}
        <div style={{ maxWidth: "1280px", margin: "0 auto", padding: "100px 2rem 64px" }}>
          <FadeIn>
        ...
        {/* Fullscreen pinned slides */}
        <div ref={prodWrapRef} style={{ height: `${FEAT_PRODUCTS.length * 100}vh`, backgroundColor: "#0A0E14" }}>
          <div style={{ position: "sticky", top: 0, height: "100vh", overflow: "hidden" }}>

            {/* Slide panels */}
            {FEAT_PRODUCTS.map((p, i) => (
              <div
                key={p.name}
                className={prodIdx === i ? "v4-prod-slide-active" : ""}
                style={{
                  position: "absolute", inset: 0,
                  display: "flex",
                  opacity:    prodIdx === i ? 1 : 0,
                  transform:  prodIdx === i ? "none" : prodIdx > i ? "translateY(-40px)" : "translateY(40px)",
              ...
              >
                ...
                <div style={{
                  ...
                  padding: "0 6% 0 8%", zIndex: 2, backgroundColor: "#0A0E14",
                }}>
                  <p style={{ fontSize: "9px", letterSpacing: "0.45em", textTransform: "uppercase", color: `${GOLD}88`, marginBottom: "18px" }}>
                    {p.num} — {p.cat}
                  </p>
                  <h3 className="v4-font-serif" style={{
                    fontSize: "clamp(3rem,6vw,5.5rem)", fontWeight: 300, color: CREAM,
                    lineHeight: 0.9, marginBottom: "28px", letterSpacing: "-0.02em",
                  }}>{p.name}</h3>
                  <div style={{ width: "40px", height: "1px", backgroundColor: `${GOLD}55`, marginBottom: "24px" }} />
                  <p style={{ color: GRAY, fontSize: "14px", lineHeight: 1.95, maxWidth: "340px", marginBottom: "36px" }}>{p.desc}</p>

                  {/* Second image thumbnail */}
                  <div style={{ width: "120px", height: "80px", overflow: "hidden", opacity: 0.6, border: `1px solid ${LINE}` }}>
                    <img src={p.img2} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }}
                      onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                  </div>
                </div>
                ...
                <div style={{ position: "absolute", bottom: "36px", left: "8%", zIndex: 10, display: "flex", alignItems: "center", gap: "12px" }}>
                  {FEAT_PRODUCTS.map((_, di) => (
                    <div key={di} style={{
                      width:  di === i ? "24px" : "6px",
                      height: "2px",
                      backgroundColor: di === i ? GOLD : `${GOLD}33`,
                      transition: "all 0.4s ease",
                      borderRadius: "1px",
                    }} />
                  ))}
